...
export const getMaterialesOrdenes = async (req, res) => {
  const [row] = await (await connect2()).query('SELECT * FROM materialestrabajosrealizados');
  res.json(row);
};
...
export const saveMaterialesTrabajos = async (req) => {
  await (
    await connect2()
  ).query(
...
export const getContadorMaterialesOrdenes = async (req, res) => {
  const [row] = await (await connect2()).query('SELECT COUNT(*) FROM materialestrabajosrealizados');
